...
import { MentalHealthCompanionIcon, LoaderIcon, AlertTriangleIcon, SendIcon } from '../components/icons';
import { chatMentalHealth } from '../services/geminiService';
import { ChatMessage } from '../types';
import { VoiceInputButton } from '../components/VoiceInputButton';

const MentalHealthCompanion: React.FC = () => {
    ...
    useEffect(() => {
        setMessages([{ sender: 'ai', text: "Hello, I'm here to listen. Feel free to share what's on your mind. Remember, I'm an AI companion and not a substitute for a real therapist." }]);
    }, []);
    ...
    const handleSendMessage = useCallback(async () => {
        ...
        try {
            const aiResponse = await chatMentalHealth(newMessages);
    ...
    return (
        <div className="h-full flex flex-col">
            <header className="flex items-center gap-4 mb-4 pb-4 border-b border-gray-200 dark:border-gray-800">
                <MentalHealthCompanionIcon className="h-10 w-10 text-indigo-500 dark:text-indigo-400" />
                <h1 className="text-4xl font-bold text-gray-900 dark:text-white">Mental Health Companion</h1>
            </header>
            ...
            <main className="flex-grow overflow-y-auto pr-2 flex flex-col gap-4 py-4">
                {messages.map((msg, index) => (
                    <div key={index} className={`flex items-start gap-3 animate-fadeInUp ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                         {msg.sender === 'ai' && <div className="bg-gray-200 dark:bg-gray-700 p-2 rounded-full mt-1"><MentalHealthCompanionIcon className="h-5 w-5 text-indigo-500 dark:text-indigo-400 flex-shrink-0" /></div>}
                ...
                {isLoading && (
                     <div className="flex justify-start items-start gap-3 animate-fadeInUp">
                        <div className="bg-gray-200 dark:bg-gray-700 p-2 rounded-full mt-1"><MentalHealthCompanionIcon className="h-5 w-5 text-indigo-500 dark:text-indigo-400 flex-shrink-0" /></div>
