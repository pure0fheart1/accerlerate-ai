...
import { TherapyJournalingPromptsIcon, LoaderIcon, AlertTriangleIcon, SparklesIcon } from '../components/icons';
import { generateJournalingPrompts } from '../services/geminiService';
import { VoiceInputButton } from '../components/VoiceInputButton';

const TherapyJournalingPrompts: React.FC = () => {
  ...
  const handleGenerate = useCallback(async () => {
    if (!topic.trim()) {
      setError("Please enter a topic or feeling to focus on.");
      return;
    }
    setIsLoading(true);
    setError(null);
    setPrompts(null);
    try {
      const result = await generateJournalingPrompts(topic);
      setPrompts(result);
    } catch (err) {
  ...
  return (
    <div className="h-full flex flex-col">
      <header className="flex items-center gap-4 mb-4">
        <TherapyJournalingPromptsIcon className="h-10 w-10 text-indigo-500 dark:text-indigo-400" />
        <h1 className="text-4xl font-bold text-gray-900 dark:text-slate-100">Journaling Prompt Generator</h1>
      </header>
      <p className="text-lg text-gray-600 dark:text-slate-400 mb-6 max-w-4xl">
        Generate thoughtful prompts to guide your journaling and self-reflection. Focus on a topic, feeling, or area of your life.
      </p>
      ...
      <div className="bg-white/70 dark:bg-slate-800/50 backdrop-blur-md border border-gray-200 dark:border-slate-700 rounded-2xl p-6 shadow-xl dark:shadow-2xl dark:shadow-black/20 mb-8 flex flex-col sm:flex-row items-center gap-4">
        <div className="relative w-full flex-grow">
          <input
            ...
            placeholder="e.g., 'practicing gratitude', 'understanding my anxiety', 'my goals for this month'"
        ...
        >
          {isLoading ? <><LoaderIcon className="h-5 w-5" /> Generating...</> : <><SparklesIcon className="h-5 w-5" /> Generate Prompts</>}
        </button>
      </div>

      <main className="flex-grow flex flex-col min-h-0">
        <h2 className="text-3xl font-bold text-gray-900 dark:text-slate-100 mb-4">Your Journaling Prompts</h2>
        <div className="w-full flex-grow bg-white/70 dark:bg-slate-800/50 backdrop-blur-md border border-gray-200 dark:border-slate-700 rounded-xl p-4 overflow-y-auto shadow-inner">
            {isLoading && (
              <div className="flex flex-col items-center justify-center h-full gap-4">
                ...
                <p className="text-indigo-600 dark:text-indigo-300">Creating thoughtful prompts...</p>
              </div>
            )}
            {error && !isLoading && (
            ...
            {!isLoading && !error && prompts && (
              <div className="prose dark:prose-invert max-w-none">{prompts}</div>
            )}
            {!isLoading && !error && !prompts && (
              <div className="flex items-center justify-center h-full text-center text-gray-500 dark:text-slate-500">
                <div>
                  <TherapyJournalingPromptsIcon className="h-24 w-24 mx-auto mb-4" />
                  <p>Your journaling prompts will appear here.</p>
                </div>
              </div>
            )}
          </div>
...
export default TherapyJournalingPrompts;
