...
import { InteriorDesignIdeasIcon, LoaderIcon, AlertTriangleIcon, SparklesIcon } from '../components/icons';
import { generateInteriorDesignIdeas } from '../services/geminiService';
import { VoiceInputButton } from '../components/VoiceInputButton';

const InteriorDesignIdeas: React.FC = () => {
  ...
  const handleGenerate = useCallback(async () => {
    if (!prompt.trim()) {
      setError("Please describe the room and your style.");
      return;
    }
    setIsLoading(true);
    setError(null);
    setIdeas(null);
    try {
      const result = await generateInteriorDesignIdeas(prompt);
      setIdeas(result);
    } catch (err) {
  ...
  return (
    <div className="h-full flex flex-col">
      <header className="flex items-center gap-4 mb-4">
        <InteriorDesignIdeasIcon className="h-10 w-10 text-indigo-500 dark:text-indigo-400" />
        <h1 className="text-4xl font-bold text-gray-900 dark:text-white">Interior Design Ideas</h1>
      </header>
      <p className="text-lg text-gray-600 dark:text-gray-400 mb-6 max-w-4xl">
        Get AI-generated interior design concepts, color palettes, and furniture suggestions based on your style preferences.
      </p>
      ...
      <div className="bg-white/70 dark:bg-gray-950/30 backdrop-blur-md border border-gray-200 dark:border-gray-800/50 rounded-2xl p-6 shadow-xl dark:shadow-2xl dark:shadow-black/20 mb-8 flex flex-col sm:flex-row items-center gap-4">
        <div className="relative w-full flex-grow">
          <input
            ...
            placeholder="e.g., 'A cozy living room with a minimalist, Scandinavian style'"
        ...
        >
          {isLoading ? <><LoaderIcon className="h-5 w-5" /> Designing...</> : <><SparklesIcon className="h-5 w-5" /> Get Ideas</>}
        </button>
      </div>

      <main className="flex-grow flex flex-col min-h-0">
        <h2 className="text-3xl font-bold text-gray-900 dark:text-white mb-4">Design Concepts</h2>
        <div className="w-full flex-grow bg-white/70 dark:bg-gray-950/30 backdrop-blur-md border border-gray-200 dark:border-gray-800/50 rounded-xl p-4 overflow-y-auto shadow-inner">
            {isLoading && (
              <div className="flex flex-col items-center justify-center h-full gap-4">
                ...
                <p className="text-indigo-600 dark:text-indigo-300">Generating design concepts...</p>
              </div>
            )}
            {error && !isLoading && (
            ...
            {!isLoading && !error && !ideas && (
              <div className="flex items-center justify-center h-full text-center text-gray-400 dark:text-gray-600">
                <div>
                  <InteriorDesignIdeasIcon className="h-24 w-24 mx-auto mb-4" />
                  <p>Your interior design ideas will appear here.</p>
                </div>
              </div>
            )}
          </div>
...
export default InteriorDesignIdeas;
