...
import { ScientificHypothesisGeneratorIcon, LoaderIcon, AlertTriangleIcon, SparklesIcon } from '../components/icons';
import { generateHypothesis } from '../services/geminiService';
import { VoiceInputButton } from '../components/VoiceInputButton';

const ScientificHypothesisGenerator: React.FC = () => {
  const [topic, setTopic] = useState('');
  const [hypotheses, setHypotheses] = useState<string | null>(null);
  ...
  return (
    <div className="h-full flex flex-col">
      <header className="flex items-center gap-4 mb-4">
        <ScientificHypothesisGeneratorIcon className="h-10 w-10 text-indigo-500 dark:text-indigo-400" />
        <h1 className="text-4xl font-bold text-gray-900 dark:text-slate-100">Scientific Hypothesis Generator</h1>
      </header>
      <p className="text-lg text-gray-600 dark:text-slate-400 mb-6 max-w-4xl">
        Generate testable scientific hypotheses based on a research area or topic of interest.
      </p>
      ...
      <div className="bg-white/70 dark:bg-slate-800/50 backdrop-blur-md border border-gray-200 dark:border-slate-700 rounded-2xl p-6 shadow-xl dark:shadow-2xl dark:shadow-black/20 mb-8 flex flex-col sm:flex-row items-center gap-4">
        <div className="relative w-full flex-grow">
          <input
            ...
            placeholder="e.g., 'The effect of music on plant growth', 'Caffeine's impact on short-term memory'"
      ...
      <main className="flex-grow flex flex-col min-h-0">
        <h2 className="text-3xl font-bold text-gray-900 dark:text-slate-100 mb-4">Testable Hypotheses</h2>
        <div className="w-full flex-grow bg-white/70 dark:bg-slate-800/50 backdrop-blur-md border border-gray-200 dark:border-slate-700 rounded-xl p-4 overflow-y-auto shadow-inner">
            {isLoading && (
              <div className="flex flex-col items-center justify-center h-full gap-4">
                ...
                <p className="text-indigo-600 dark:text-indigo-300">Formulating hypotheses...</p>
              </div>
            )}
            {error && !isLoading && (
            ...
            {!isLoading && !error && hypotheses && (
              <div className="prose dark:prose-invert max-w-none">{hypotheses}</div>
            )}
            {!isLoading && !error && !hypotheses && (
              <div className="flex items-center justify-center h-full text-center text-gray-500 dark:text-slate-500">
                <div>
                  <ScientificHypothesisGeneratorIcon className="h-24 w-24 mx-auto mb-4" />
                  <p>Your generated hypotheses will appear here.</p>
                </div>
              </div>
            )}
          </div>
...
export default ScientificHypothesisGenerator;
