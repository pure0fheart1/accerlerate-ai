...
import { ResearchPaperSummarizerIcon, LoaderIcon, AlertTriangleIcon, SparklesIcon } from '../components/icons';
...
const ResearchPaperSummarizer: React.FC = () => {
  ...
  const handleGenerate = useCallback(async () => {
    if (!text.trim()) {
      setError("Please paste the text of the research paper.");
      return;
    }
    setIsLoading(true);
    setError(null);
    setSummary(null);
    try {
      const result = await summarizeResearchPaper(text);
      setSummary(result);
    } catch (err) {
  ...
  return (
    <div className="h-full flex flex-col">
      <header className="flex items-center gap-4 mb-4">
        <ResearchPaperSummarizerIcon className="h-10 w-10 text-indigo-500 dark:text-indigo-400" />
        <h1 className="text-4xl font-bold text-gray-900 dark:text-white">Research Paper Summarizer</h1>
      </header>
      <p className="text-lg text-gray-600 dark:text-gray-400 mb-6 max-w-4xl">
        Distill dense academic papers into concise summaries of key findings, methodologies, and conclusions.
      </p>
      ...
      <main className="flex-grow grid grid-cols-1 lg:grid-cols-2 gap-8 min-h-0">
        <div className="bg-white/70 dark:bg-gray-950/30 backdrop-blur-md border border-gray-200 dark:border-gray-800/50 rounded-2xl p-6 shadow-xl dark:shadow-2xl dark:shadow-black/20 flex flex-col">
          <h2 className="text-2xl font-semibold text-gray-900 dark:text-white mb-4">Paper Text</h2>
          ...
          >
            {isLoading ? <><LoaderIcon className="h-5 w-5" /> Summarizing...</> : <><SparklesIcon className="h-5 w-5" /> Summarize Paper</>}
          </button>
        </div>
        ...
        <div className="bg-white/70 dark:bg-gray-950/30 backdrop-blur-md border border-gray-200 dark:border-gray-800/50 rounded-2xl p-6 shadow-xl dark:shadow-2xl dark:shadow-black/20 flex flex-col min-h-0">
          ...
          <div className="w-full flex-grow bg-gray-50 dark:bg-black/20 rounded-xl p-4 overflow-y-auto border border-gray-200 dark:border-gray-800">
            {isLoading && (
              <div className="flex flex-col items-center justify-center h-full gap-4"><LoaderIcon className="h-12 w-12 text-indigo-500 dark:text-indigo-400" /><p className="text-indigo-600 dark:text-indigo-300">Distilling the research...</p></div>
            )}
            {error && !isLoading && (
            ...
            {!isLoading && !error && !summary && (
              <div className="flex items-center justify-center h-full text-center text-gray-400 dark:text-gray-600">
                <div><ResearchPaperSummarizerIcon className="h-24 w-24 mx-auto mb-4" /><p>The research summary will appear here.</p></div>
              </div>
            )}
          </div>
        </div>
