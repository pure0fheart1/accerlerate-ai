...
import { PitchDeckCreatorIcon, LoaderIcon, AlertTriangleIcon, SparklesIcon } from '../components/icons';
import { createPitchDeck } from '../services/geminiService';
import { PitchDeckSlide } from '../types';
import { VoiceInputButton } from '../components/VoiceInputButton';

const PitchDeckCreator: React.FC = () => {
  const [idea, setIdea] = useState('');
  const [deck, setDeck] = useState<PitchDeckSlide[]>([]);
  ...
  const handleGenerate = useCallback(async () => {
    ...
    setDeck([]);
    try {
      const result = await createPitchDeck(idea);
      setDeck(result);
    } catch (err) {
  ...
  return (
    <div className="h-full flex flex-col">
      <header className="flex items-center gap-4 mb-4">
        <PitchDeckCreatorIcon className="h-10 w-10 text-indigo-500 dark:text-indigo-400" />
        <h1 className="text-4xl font-bold text-gray-900 dark:text-white">Pitch Deck Creator</h1>
      </header>
      <p className="text-lg text-gray-600 dark:text-gray-400 mb-6 max-w-4xl">
        Generate a structured 10-slide pitch deck outline for your startup idea.
      </p>
      ...
      <main className="flex-grow grid grid-cols-1 lg:grid-cols-3 gap-8 min-h-0">
        <div className="lg:col-span-1 bg-white/70 dark:bg-gray-950/30 backdrop-blur-md border border-gray-200 dark:border-gray-800/50 rounded-2xl p-6 shadow-xl dark:shadow-2xl dark:shadow-black/20 flex flex-col gap-6">
          ...
          <div className="relative flex-grow">
            ...
            <div className="relative h-full">
                <textarea
                  ...
                  placeholder="e.g., A platform that connects freelance writers with small businesses for content creation."
          ...
          >
            {isLoading ? <><LoaderIcon className="h-5 w-5" /> Creating...</> : <><SparklesIcon className="h-5 w-5" /> Create Deck Outline</>}
          </button>
        </div>
        ...
        <div className="lg:col-span-2 bg-white/70 dark:bg-gray-950/30 backdrop-blur-md border border-gray-200 dark:border-gray-800/50 rounded-2xl p-6 shadow-xl dark:shadow-2xl dark:shadow-black/20 flex flex-col min-h-0">
          <h2 className="text-2xl font-semibold text-gray-900 dark:text-white mb-4">Pitch Deck Outline</h2>
          <div className="w-full flex-grow bg-gray-50 dark:bg-black/20 rounded-xl p-4 overflow-y-auto border border-gray-200 dark:border-gray-800">
            {isLoading && (
              <div className="flex flex-col items-center justify-center h-full gap-4"><LoaderIcon className="h-12 w-12 text-indigo-500 dark:text-indigo-400" /><p className="text-indigo-600 dark:text-indigo-300">Building your pitch deck...</p></div>
            )}
            {error && !isLoading && (
            ...
            {!isLoading && !error && deck.length > 0 && (
              <div className="space-y-4">
                {deck.map((slide) => (
                  <div key={slide.slideNumber} className="bg-white/50 dark:bg-gray-900/50 p-4 rounded-lg border border-gray-200 dark:border-gray-700">
                    <h3 className="text-lg font-bold text-indigo-600 dark:text-indigo-400">Slide {slide.slideNumber}: {slide.title}</h3>
            ...
            {!isLoading && !error && deck.length === 0 && (
              <div className="flex items-center justify-center h-full text-center text-gray-400 dark:text-gray-600">
                <div><PitchDeckCreatorIcon className="h-24 w-24 mx-auto mb-4" /><p>Your pitch deck outline will appear here.</p></div>
              </div>
            )}
          </div>
        </div>
