...
import { ChatBubbleBottomCenterTextIcon, LoaderIcon, AlertTriangleIcon, SendIcon } from '../components/icons';
import { generateText } from '../services/geminiService';
import { ChatMessage } from '../types';
import { VoiceInputButton } from '../components/VoiceInputButton';

const MESSAGES_KEY = 'accelerate-aitutor-messages';
...
const AITutor: React.FC = () => {
    ...
    const tutorInstruction = "You are a friendly and knowledgeable AI Tutor. Your goal is to explain concepts clearly and concisely. Answer the user's questions, break down complex topics into simple terms, and provide examples when helpful. Encourage learning and curiosity.";
    ...
    useEffect(() => {
        try {
            ...
            if (storedMessages) {
                ...
                setMessages([{ sender: 'ai', text: "Hello! I'm your AI Tutor. What would you like to learn about today?" }]);
            }
        } catch (e) {
    ...
    const handleSendMessage = useCallback(async () => {
        ...
        try {
            const aiResponse = await generateText(tutorInstruction, currentInput);
    ...
    return (
        <div className="h-full flex flex-col">
            <header className="flex items-center gap-4 mb-4 pb-4 border-b border-gray-200 dark:border-slate-700">
                <ChatBubbleBottomCenterTextIcon className="h-10 w-10 text-indigo-500 dark:text-indigo-400" />
                <h1 className="text-4xl font-bold text-gray-900 dark:text-slate-100">AI Tutor</h1>
            </header>
            ...
            <main className="flex-grow overflow-y-auto pr-2 flex flex-col gap-4 py-4">
                {messages.map((msg, index) => (
                    <div key={index} className={`flex items-start gap-3 animate-fadeInUp ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                         {msg.sender === 'ai' && <div className="bg-gray-200 dark:bg-slate-700 p-2 rounded-full mt-1"><ChatBubbleBottomCenterTextIcon className="h-5 w-5 text-indigo-500 dark:text-indigo-400 flex-shrink-0" /></div>}
                ...
                {isLoading && (
                     <div className="flex justify-start items-start gap-3 animate-fadeInUp">
                        <div className="bg-gray-200 dark:bg-slate-700 p-2 rounded-full mt-1"><ChatBubbleBottomCenterTextIcon className="h-5 w-5 text-indigo-500 dark:text-indigo-400 flex-shrink-0" /></div>
