...
import { HaikuGeneratorIcon, LoaderIcon, AlertTriangleIcon, SparklesIcon } from '../components/icons';
  ...
  return (
    <div className="h-full flex flex-col">
      <header className="flex items-center gap-4 mb-4">
        <HaikuGeneratorIcon className="h-10 w-10 text-indigo-500 dark:text-indigo-400" />
        <h1 className="text-4xl font-bold text-gray-900 dark:text-slate-100">Haiku Generator</h1>
      </header>
      <p className="text-lg text-gray-600 dark:text-slate-400 mb-6 max-w-4xl">
        Craft a beautiful 5-7-5 syllable haiku poem from any topic you can imagine.
      </p>
      ...
      <div className="bg-white/70 dark:bg-slate-800/50 backdrop-blur-md border border-gray-200 dark:border-slate-700 rounded-2xl p-6 shadow-xl dark:shadow-2xl dark:shadow-black/20 mb-8 flex flex-col sm:flex-row items-center gap-4">
        <div className="relative w-full flex-grow">
          <input
            ...
            placeholder="e.g., 'A quiet winter morning', 'Cherry blossoms in spring'"
        ...
        >
          {isLoading ? <><LoaderIcon className="h-5 w-5" /> Writing...</> : <><SparklesIcon className="h-5 w-5" /> Generate Haiku</>}
        </button>
      </div>

      <main className="flex-grow flex flex-col min-h-0">
        <h2 className="text-3xl font-bold text-gray-900 dark:text-slate-100 mb-4">Generated Haiku</h2>
        <div className="w-full flex-grow bg-white/70 dark:bg-slate-800/50 backdrop-blur-md border border-gray-200 dark:border-slate-700 rounded-xl p-4 overflow-y-auto shadow-inner flex items-center justify-center">
            {isLoading && (
              <div className="flex flex-col items-center justify-center h-full gap-4">
                <LoaderIcon className="h-12 w-12 text-indigo-500 dark:text-indigo-400" />
                <p className="text-indigo-600 dark:text-indigo-300">Finding poetic words...</p>
              </div>
            )}
            {error && !isLoading && (
            ...
            {!isLoading && !error && haiku && (
              <div className="prose dark:prose-invert max-w-none text-center">
                <p className="text-2xl leading-loose whitespace-pre-wrap">{haiku}</p>
              </div>
            )}
            {!isLoading && !error && !haiku && (
              <div className="flex items-center justify-center h-full text-center text-gray-500 dark:text-slate-500">
                <div>
                  <HaikuGeneratorIcon className="h-24 w-24 mx-auto mb-4" />
                  <p>Your generated haiku will appear here.</p>
                </div>
              </div>
            )}
          </div>
