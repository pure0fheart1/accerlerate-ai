...
import { FitnessCoachIcon, LoaderIcon, AlertTriangleIcon, SendIcon } from '../components/icons';
...
const FitnessCoach: React.FC = () => {
    ...
    useEffect(() => {
        setMessages([{ sender: 'ai', text: "Hello! I'm your AI Fitness Coach. I can help with workout suggestions, nutrition questions, and general fitness advice. What's on your mind today? \n\n*Disclaimer: I am not a medical professional. Please consult a doctor before starting any new fitness program.*" }]);
    }, []);
    ...
    const handleSendMessage = useCallback(async () => {
        ...
        try {
            const aiResponse = await getFitnessCoaching(newMessages);
    ...
    return (
        <div className="h-full flex flex-col">
            <header className="flex items-center gap-4 mb-4 pb-4 border-b border-gray-200 dark:border-gray-800">
                <FitnessCoachIcon className="h-10 w-10 text-indigo-500 dark:text-indigo-400" />
                <h1 className="text-4xl font-bold text-gray-900 dark:text-white">AI Fitness Coach</h1>
            </header>
            ...
            <main className="flex-grow overflow-y-auto pr-2 flex flex-col gap-4 py-4">
                {messages.map((msg, index) => (
                    <div key={index} className={`flex items-start gap-3 animate-fadeInUp ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                         {msg.sender === 'ai' && <div className="bg-gray-200 dark:bg-gray-700 p-2 rounded-full mt-1"><FitnessCoachIcon className="h-5 w-5 text-indigo-500 dark:text-indigo-400 flex-shrink-0" /></div>}
                ...
                {isLoading && (
                     <div className="flex justify-start items-start gap-3 animate-fadeInUp">
                        <div className="bg-gray-200 dark:bg-gray-700 p-2 rounded-full mt-1"><FitnessCoachIcon className="h-5 w-5 text-indigo-500 dark:text-indigo-400 flex-shrink-0" /></div>
            ...
            <footer className="mt-auto pt-4">
                ...
                <div className="relative flex items-center gap-3">
                     <textarea
                        ...
                        placeholder="Ask for workout ideas, nutrition tips, etc."
