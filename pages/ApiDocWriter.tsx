...
import { ApiDocWriterIcon, LoaderIcon, AlertTriangleIcon, SparklesIcon } from '../components/icons';
import { writeApiDocs } from '../services/geminiService';
import { VoiceInputButton } from '../components/VoiceInputButton';

const CODE_KEY = 'accelerate-apidoc-code';
const DOCS_KEY = 'accelerate-apidoc-output';

const ApiDocWriter: React.FC = () => {
  const [code, setCode] = useState(() => localStorage.getItem(CODE_KEY) || '');
  const [docs, setDocs] = useState<string | null>(() => localStorage.getItem(DOCS_KEY) || null);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => { localStorage.setItem(CODE_KEY, code); }, [code]);
  useEffect(() => {
    if (docs) localStorage.setItem(DOCS_KEY, docs);
    else localStorage.removeItem(DOCS_KEY);
  }, [docs]);

  const handleGenerate = useCallback(async () => {
    if (!code.trim()) {
      setError("Please paste the code for which you want to generate documentation.");
      return;
    }
    setIsLoading(true);
    setError(null);
    setDocs(null);
    try {
      const result = await writeApiDocs(code);
      setDocs(result);
    } catch (err) {
  ...
  return (
    <div className="h-full flex flex-col">
      <header className="flex items-center gap-4 mb-4">
        <ApiDocWriterIcon className="h-10 w-10 text-indigo-500 dark:text-indigo-400" />
        <h1 className="text-4xl font-bold text-gray-900 dark:text-white">API Doc Writer</h1>
      </header>
      <p className="text-lg text-gray-600 dark:text-gray-400 mb-6 max-w-4xl">
        Automatically generate clear and concise documentation for your code functions and APIs.
      </p>
      ...
      <main className="flex-grow grid grid-cols-1 lg:grid-cols-2 gap-8 min-h-0">
        <div className="bg-white/70 dark:bg-gray-950/30 backdrop-blur-md border border-gray-200 dark:border-gray-800/50 rounded-2xl p-6 shadow-xl dark:shadow-2xl dark:shadow-black/20 flex flex-col">
          <h2 className="text-2xl font-semibold text-gray-900 dark:text-white mb-4">Code / Function</h2>
          <div className="relative w-full flex-grow">
            <textarea
              value={code}
              onChange={(e) => setCode(e.target.value)}
              placeholder={`function calculateTotal(items, taxRate) {\n  // ...\n}`}
          ...
          >
            {isLoading ? <><LoaderIcon className="h-5 w-5" /> Documenting...</> : <><SparklesIcon className="h-5 w-5" /> Generate Docs</>}
          </button>
        </div>
        ...
        <div className="bg-white/70 dark:bg-gray-950/30 backdrop-blur-md border border-gray-200 dark:border-gray-800/50 rounded-2xl p-6 shadow-xl dark:shadow-2xl dark:shadow-black/20 flex flex-col min-h-0">
          <h2 className="text-2xl font-semibold text-gray-900 dark:text-white mb-4">Generated Documentation</h2>
          <div className="w-full flex-grow bg-gray-50 dark:bg-black/20 rounded-xl p-4 overflow-y-auto border border-gray-200 dark:border-gray-800">
            {isLoading && (
              <div className="flex flex-col items-center justify-center h-full gap-4"><LoaderIcon className="h-12 w-12 text-indigo-500 dark:text-indigo-400" /><p className="text-indigo-600 dark:text-indigo-300">Writing documentation...</p></div>
            )}
            {error && !isLoading && (
            ...
            {!isLoading && !error && docs && (
              <div className="prose dark:prose-invert max-w-none whitespace-pre-wrap">{docs}</div>
            )}
            {!isLoading && !error && !docs && (
              <div className="flex items-center justify-center h-full text-center text-gray-400 dark:text-gray-600">
                <div><ApiDocWriterIcon className="h-24 w-24 mx-auto mb-4" /><p>Your generated API documentation will appear here.</p></div>
              </div>
            )}
          </div>
        </div>
