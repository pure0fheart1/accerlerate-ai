...
import { AcronymExplainerIcon, LoaderIcon, AlertTriangleIcon, SparklesIcon } from '../components/icons';
import { explainAcronym } from '../services/geminiService';
import { VoiceInputButton } from '../components/VoiceInputButton';

const AcronymExplainer: React.FC = () => {
  ...
  const handleGenerate = useCallback(async () => {
    if (!acronym.trim()) {
      setError("Please enter an acronym to explain.");
      return;
    }
    setIsLoading(true);
    setError(null);
    setExplanation(null);
    try {
      const result = await explainAcronym(acronym);
  ...
  const isGenerateDisabled = useMemo(() => isLoading || !acronym.trim(), [isLoading, acronym]);

  return (
    <div className="h-full flex flex-col">
      <header className="flex items-center gap-4 mb-4">
        <AcronymExplainerIcon className="h-10 w-10 text-indigo-500 dark:text-indigo-400" />
        <h1 className="text-4xl font-bold text-gray-900 dark:text-slate-100">Acronym Explainer</h1>
      </header>
      <p className="text-lg text-gray-600 dark:text-slate-400 mb-6 max-w-4xl">
        Decode any acronym or abbreviation. Get clear explanations for industry jargon, tech terms, and more.
      </p>
      ...
      <div className="bg-white/70 dark:bg-slate-800/50 backdrop-blur-md border border-gray-200 dark:border-slate-700 rounded-2xl p-6 shadow-xl dark:shadow-2xl dark:shadow-black/20 mb-8 flex flex-col sm:flex-row items-center gap-4">
        <div className="relative w-full flex-grow">
          <input
            ...
            aria-label="Acronym to explain"
          />
           <div className="absolute inset-y-0 right-0 flex items-center pr-3">
            <VoiceInputButton onTranscript={(t) => setAcronym(current => (current ? current.trim() + ' ' : '') + t)} />
          </div>
        </div>
        <button
        ...
        <div className="w-full flex-grow bg-white/70 dark:bg-slate-800/50 backdrop-blur-md border border-gray-200 dark:border-slate-700 rounded-xl p-4 overflow-y-auto shadow-inner">
            {isLoading && (
              <div className="flex flex-col items-center justify-center h-full gap-4">
                ...
                <p className="text-indigo-600 dark:text-indigo-300">Looking it up...</p>
              </div>
            )}
            {error && !isLoading && (
            ...
            {!isLoading && !error && !explanation && (
              <div className="flex items-center justify-center h-full text-center text-gray-500 dark:text-slate-500">
                <div>
                  <AcronymExplainerIcon className="h-24 w-24 mx-auto mb-4" />
                  <p>The explanation for your acronym will appear here.</p>
                </div>
              </div>
            )}
          </div>
...
export default AcronymExplainer;
