...
import { LiteraryDeviceIdentifierIcon, LoaderIcon, AlertTriangleIcon, SparklesIcon } from '../components/icons';
import { identifyLiteraryDevice } from '../services/geminiService';
import { VoiceInputButton } from '../components/VoiceInputButton';

const LiteraryDeviceIdentifier: React.FC = () => {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    setAnalysis(null);
    try {
      const result = await identifyLiteraryDevice(text);
      setAnalysis(result);
    } catch (err) {
  ...
  return (
    <div className="h-full flex flex-col">
      <header className="flex items-center gap-4 mb-4">
        <LiteraryDeviceIdentifierIcon className="h-10 w-10 text-indigo-500 dark:text-indigo-400" />
        <h1 className="text-4xl font-bold text-gray-900 dark:text-slate-100">Literary Device Identifier</h1>
      </header>
      <p className="text-lg text-gray-600 dark:text-slate-400 mb-6 max-w-4xl">
        Analyze poetry or prose to identify and understand the literary devices at play. Paste a piece of text to get a detailed breakdown.
      </p>
        ...
        <div className="bg-white/70 dark:bg-slate-800/50 backdrop-blur-md border border-gray-200 dark:border-slate-700 rounded-2xl p-6 shadow-xl dark:shadow-2xl dark:shadow-black/20 flex flex-col">
          ...
          <div className="relative w-full flex-grow">
            <textarea
              ...
              placeholder="Paste a poem, a paragraph, or any literary text here..."
              ...
              aria-label="Text for literary device analysis"
            />
          ...
          >
            {isLoading ? <><LoaderIcon className="h-5 w-5" /> Analyzing...</> : <><SparklesIcon className="h-5 w-5" /> Analyze Text</>}
          </button>
        </div>
          ...
          <div className="w-full flex-grow bg-gray-50 dark:bg-slate-900 rounded-xl p-4 overflow-y-auto border border-gray-200 dark:border-slate-700">
            {isLoading && (
              <div className="flex flex-col items-center justify-center h-full gap-4">
                ...
                <p className="text-indigo-600 dark:text-indigo-300">Analyzing literary devices...</p>
              </div>
            )}
            {error && !isLoading && (
            ...
            {!isLoading && !error && !analysis && (
              <div className="flex items-center justify-center h-full text-center text-gray-500 dark:text-slate-500">
                <div>
                  <LiteraryDeviceIdentifierIcon className="h-24 w-24 mx-auto mb-4" />
                  <p>The analysis of literary devices will appear here.</p>
                </div>
              </div>
            )}
          </div>
...
export default LiteraryDeviceIdentifier;