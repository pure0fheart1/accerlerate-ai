...
import { StartScreen } from '../components/fitcheck/StartScreen';
import { Canvas } from '../components/fitcheck/Canvas';
import WardrobePanel from '../components/fitcheck/WardrobePanel';
import OutfitStack from '../components/fitcheck/OutfitStack';
...
import { ChevronDownIcon, ChevronUpIcon } from '../components/fitcheck/icons';
import { defaultWardrobe } from '../data/wardrobe';
import { getFriendlyErrorMessage } from '../lib/utils';
import { Spinner } from '../components/fitcheck/Spinner';
import { saveFitCheckState, loadFitCheckState, clearFitCheckState } from '../lib/fitcheckStorage';
...
const FitCheck: React.FC = () => {
  ...
  useEffect(() => {
    const loadState = async () => {
      const savedState = await loadFitCheckState();
      if (savedState) {
        setModelImageUrl(savedState.modelImageUrl);
        setOutfitHistory(savedState.outfitHistory);
        setCurrentOutfitIndex(savedState.currentOutfitIndex);
        setCurrentPoseIndex(savedState.currentPoseIndex);
        setWardrobe(savedState.wardrobe.length > 0 ? savedState.wardrobe : defaultWardrobe);
      }
  ...
  useEffect(() => {
    ...
    const saveState = async () => {
      try {
        ...
        console.error('Error saving FitCheck state:', error);
      }
    };

    saveState();
  }, [modelImageUrl, outfitHistory, currentOutfitIndex, currentPoseIndex, wardrobe, isInitialized]);
