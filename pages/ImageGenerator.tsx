...
const saveToGallery = (imageUrl: string, prompt: string) => {
  try {
...
const ImageGenerator: React.FC<ImageGeneratorProps> = ({ sharedPrompt, setSharedPrompt }) => {
  ...
  const [layout, setLayout] = useState<'default' | 'compact' | 'wide'>('default');

  // Collapsible sections state
  const [isPromptExpanded, setIsPromptExpanded] = useState(true);
  const [isStylesExpanded, setIsStylesExpanded] = useState(true);
  const [isNumImagesExpanded, setIsNumImagesExpanded] = useState(true);
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      result.forEach(imageUrl => {
        saveToGallery(imageUrl, finalPrompt);
      });
    } catch (err: unknown) {
  ...
  const getLayoutClasses = () => {
    switch(layout) {
      ...
      default:
        return 'grid-cols-1 xl:grid-cols-3 gap-6 md:gap-8 lg:gap-12';
    }
  };
  ...
  return (
    <>
      <div className="min-h-full flex flex-col max-w-7xl mx-auto w-full">
          {/* Collapsible Header with Layout Options */}
          <header className="text-center mb-6 px-4 flex-shrink-0">
              ...
              >
                ...
                <svg
                  className={`w-8 h-8 text-gray-600 dark:text-slate-400 transition-transform duration-200 flex-shrink-0 ml-4 ${isHeaderExpanded ? 'transform rotate-180' : ''}`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
              ...
              {isHeaderExpanded && (
                <>
                  <p className="text-xl lg:text-2xl text-gray-600 dark:text-slate-400 font-light max-w-3xl mx-auto mb-6">Transform your ideas into stunning visuals with AI</p>

                  {/* Layout Options */}
                  <div className="flex items-center justify-center gap-3 mb-4">
                    <span className="text-sm text-gray-600 dark:text-slate-400 font-medium">Layout:</span>
          ...
          <main className={`flex-grow grid ${getLayoutClasses()} min-h-0 px-4 pb-8`}>
            ...
            <div className="xl:col-span-1 space-y-8">
              ...
              <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm border border-gray-200 dark:border-slate-700 rounded-3xl p-6 md:p-8 shadow-lg hover:shadow-xl transition-all duration-300 group">
                ...
                >
                  ...
                  <svg
                    className={`w-6 h-6 text-gray-600 dark:text-slate-400 transition-transform duration-200 ${isPromptExpanded ? 'transform rotate-180' : ''}`}
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
              ...
              <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm border border-gray-200 dark:border-slate-700 rounded-3xl p-6 md:p-8 shadow-lg hover:shadow-xl transition-all duration-300 relative z-30 overflow-visible">
                <button
                ...
                >
                  ...
                  <svg
                    className={`w-6 h-6 text-gray-600 dark:text-slate-400 transition-transform duration-200 ${isStylesExpanded ? 'transform rotate-180' : ''}`}
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
                ...
                {isStylesExpanded && (
                  <div className="relative z-40" ref={styleInputRef}>
                    ...
                    {isStyleDropdownOpen && filteredStyles.length > 0 && (
                      <div className="absolute z-[100] w-full mt-1 bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-600 rounded-2xl shadow-xl max-h-64 overflow-y-auto">
              ...
              <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm border border-gray-200 dark:border-slate-700 rounded-3xl p-6 md:p-8 shadow-lg hover:shadow-xl transition-all duration-300">
                ...
                >
                  ...
                  <svg
                    className={`w-6 h-6 text-gray-600 dark:text-slate-400 transition-transform duration-200 ${isNumImagesExpanded ? 'transform rotate-180' : ''}`}
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
            ...
            <div className="xl:col-span-2 grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
              ...
              <div className="lg:col-span-2 bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm border border-gray-200 dark:border-slate-700 rounded-3xl p-6 md:p-8 shadow-lg hover:shadow-xl transition-all duration-300">
                ...
                >
                  <h2 className="text-3xl font-semibold text-gray-900 dark:text-slate-100 flex items-center gap-4">
                    <ImageIcon className="h-8 w-8 text-indigo-500 dark:text-indigo-400" />
                    Generated Images
                  </h2>
                  <svg
                    className={`w-6 h-6 text-gray-600 dark:text-slate-400 transition-transform duration-200 ${isResultsExpanded ? 'transform rotate-180' : ''}`}
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
              ...
              <div className="lg:col-span-1 bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm border border-gray-200 dark:border-slate-700 rounded-3xl p-6 md:p-8 shadow-lg hover:shadow-xl transition-all duration-300">
                <div className="flex justify-between items-center mb-6">
                    ...
                    >
                      ...
                      <svg
                        className={`w-5 h-5 text-gray-600 dark:text-slate-400 transition-transform duration-200 ${isHistoryExpanded ? 'transform rotate-180' : ''}`}
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                      >
