...
import { ConflictResolutionAdvisorIcon, LoaderIcon, AlertTriangleIcon, SparklesIcon } from '../components/icons';
import { adviseOnConflict } from '../services/geminiService';
import { VoiceInputButton } from '../components/VoiceInputButton';

const ConflictResolutionAdvisor: React.FC = () => {
  ...
  const handleGenerate = useCallback(async () => {
    if (!situation.trim()) {
      setError("Please describe the conflict situation.");
      return;
    }
    setIsLoading(true);
    setError(null);
    setAdvice(null);
    try {
      const result = await adviseOnConflict(situation);
      setAdvice(result);
    } catch (err) {
  ...
  const isGenerateDisabled = useMemo(() => isLoading || !situation.trim(), [isLoading, situation]);

  return (
    <div className="h-full flex flex-col">
      <header className="flex items-center gap-4 mb-4">
        <ConflictResolutionAdvisorIcon className="h-10 w-10 text-indigo-500 dark:text-indigo-400" />
        <h1 className="text-4xl font-bold text-gray-900 dark:text-slate-100">Conflict Resolution Advisor</h1>
      </header>
      <p className="text-lg text-gray-600 dark:text-slate-400 mb-6 max-w-4xl">
        Navigate difficult conversations with confidence. Describe a conflict, and get AI-powered advice on how to approach it constructively, including sample phrasing.
      </p>
      ...
      <main className="flex-grow grid grid-cols-1 lg:grid-cols-2 gap-8 min-h-0">
        <div className="bg-white/70 dark:bg-slate-800/50 backdrop-blur-md border border-gray-200 dark:border-slate-700 rounded-2xl p-6 shadow-xl dark:shadow-2xl dark:shadow-black/20 flex flex-col">
          <h2 className="text-2xl font-semibold text-gray-900 dark:text-slate-100 mb-4">The Situation</h2>
          <div className="relative w-full flex-grow">
            <textarea
              value={situation}
              onChange={(e) => setSituation(e.target.value)}
              placeholder="e.g., 'My coworker is consistently taking credit for my work in team meetings.'"
              ...
              aria-label="Conflict situation description"
            />
            <div className="absolute bottom-3 right-3">
                <VoiceInputButton onTranscript={(t) => setSituation(current => (current ? current.trim() + ' ' : '') + t)} />
            </div>
          </div>
          <button
          ...
          >
            {isLoading ? <><LoaderIcon className="h-5 w-5" /> Analyzing...</> : <><SparklesIcon className="h-5 w-5" /> Get Advice</>}
          </button>
        </div>
        ...
        <div className="bg-white/70 dark:bg-slate-800/50 backdrop-blur-md border border-gray-200 dark:border-slate-700 rounded-2xl p-6 shadow-xl dark:shadow-2xl dark:shadow-black/20 flex flex-col min-h-0">
          <h2 className="text-2xl font-semibold text-gray-900 dark:text-slate-100 mb-4">Constructive Approach</h2>
          <div className="w-full flex-grow bg-gray-50 dark:bg-slate-900 rounded-xl p-4 overflow-y-auto border border-gray-200 dark:border-slate-700">
            {isLoading && (
              <div className="flex flex-col items-center justify-center h-full gap-4">
                ...
                <p className="text-indigo-600 dark:text-indigo-300">Finding a path to resolution...</p>
              </div>
            )}
            {error && !isLoading && (
            ...
            {!isLoading && !error && !advice && (
              <div className="flex items-center justify-center h-full text-center text-gray-500 dark:text-slate-500">
                <div>
                  <ConflictResolutionAdvisorIcon className="h-24 w-24 mx-auto mb-4" />
                  <p>Your conflict resolution advice will appear here.</p>
                </div>
              </div>
            )}
          </div>
...
export default ConflictResolutionAdvisor;
