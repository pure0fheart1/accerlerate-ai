...
import { HobbySuggesterIcon, LoaderIcon, AlertTriangleIcon, SparklesIcon } from '../components/icons';
import { suggestHobbies } from '../services/geminiService';
import { VoiceInputButton } from '../components/VoiceInputButton';

const HobbySuggester: React.FC = () => {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    setSuggestions(null);
    try {
      const result = await suggestHobbies(interests);
  ...
  const isGenerateDisabled = useMemo(() => isLoading || !interests.trim(), [isLoading, interests]);

  return (
    <div className="h-full flex flex-col">
      <header className="flex items-center gap-4 mb-4">
        <HobbySuggesterIcon className="h-10 w-10 text-indigo-500 dark:text-indigo-400" />
        <h1 className="text-4xl font-bold text-gray-900 dark:text-slate-100">Hobby Suggester</h1>
      </header>
      <p className="text-lg text-gray-600 dark:text-slate-400 mb-6 max-w-4xl">
        Discover new passions. Describe your interests, and let AI suggest hobbies you might love.
      </p>
      ...
      <div className="bg-white/70 dark:bg-slate-800/50 backdrop-blur-md border border-gray-200 dark:border-slate-700 rounded-2xl p-6 shadow-xl dark:shadow-2xl dark:shadow-black/20 mb-8 flex flex-col sm:flex-row items-center gap-4">
        <div className="relative w-full flex-grow">
          <input
            ...
            placeholder="e.g., 'I like being outdoors, technology, and quiet activities'"
          ...
          />
           <div className="absolute inset-y-0 right-0 flex items-center pr-3">
            <VoiceInputButton onTranscript={(t) => setInterests(current => (current ? current.trim() + ' ' : '') + t)} />
          </div>
        </div>
        <button
        ...
        >
          {isLoading ? <><LoaderIcon className="h-5 w-5" /> Suggesting...</> : <><SparklesIcon className="h-5 w-5" /> Suggest Hobbies</>}
        </button>
      </div>

      <main className="flex-grow flex flex-col min-h-0">
        <h2 className="text-3xl font-bold text-gray-900 dark:text-slate-100 mb-4">Hobby Suggestions</h2>
        <div className="w-full flex-grow bg-white/70 dark:bg-slate-800/50 backdrop-blur-md border border-gray-200 dark:border-slate-700 rounded-xl p-4 overflow-y-auto shadow-inner">
            {isLoading && (
              <div className="flex flex-col items-center justify-center h-full gap-4">
                ...
                <p className="text-indigo-600 dark:text-indigo-300">Finding your next passion...</p>
              </div>
            )}
            {error && !isLoading && (
            ...
            {!isLoading && !error && !suggestions && (
              <div className="flex items-center justify-center h-full text-center text-gray-500 dark:text-slate-500">
                <div>
                  <HobbySuggesterIcon className="h-24 w-24 mx-auto mb-4" />
                  <p>Your personalized hobby suggestions will appear here.</p>
                </div>
              </div>
            )}
          </div>
