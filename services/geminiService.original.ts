...
let API_KEY: string | undefined;let ai: GoogleGenAI | undefined;try {  API_KEY = import.meta.env.VITE_API_KEY || process.env.API_KEY;  if (API_KEY) {    ai = new GoogleGenAI({ apiKey: API_KEY });  }} catch (error) {  console.warn("Failed to initialize Google AI client:", error);}const getAI = () => {  if (!ai || !API_KEY) {    throw new Error("API_KEY environment variable not set. Please configure VITE_API_KEY in your environment.");  }  return ai;};
