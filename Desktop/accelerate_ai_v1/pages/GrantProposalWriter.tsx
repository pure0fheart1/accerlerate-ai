...
import { GrantProposalWriterIcon, LoaderIcon, AlertTriangleIcon, SparklesIcon } from '../components/icons';
import { writeGrantProposal } from '../services/geminiService';
import { VoiceInputButton } from '../components/VoiceInputButton';

const GrantProposalWriter: React.FC = () => {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    setProposal(null);
    try {
      const result = await writeGrantProposal(description);
      setProposal(result);
    } catch (err) {
  ...
  return (
    <div className="h-full flex flex-col">
      <header className="flex items-center gap-4 mb-4">
        <GrantProposalWriterIcon className="h-10 w-10 text-indigo-500 dark:text-indigo-400" />
        <h1 className="text-4xl font-bold text-gray-900 dark:text-white">Grant Proposal Writer</h1>
      </header>
      <p className="text-lg text-gray-600 dark:text-gray-400 mb-6 max-w-4xl">
        Draft compelling grant proposals to secure funding for your projects.
      </p>
      ...
      <main className="flex-grow grid grid-cols-1 lg:grid-cols-2 gap-8 min-h-0">
        <div className="bg-white/70 dark:bg-gray-950/30 backdrop-blur-md border border-gray-200 dark:border-gray-800/50 rounded-2xl p-6 shadow-xl dark:shadow-2xl dark:shadow-black/20 flex flex-col">
          <h2 className="text-2xl font-semibold text-gray-900 dark:text-white mb-4">Project Description</h2>
          <div className="relative w-full flex-grow">
            <textarea
              value={description}
              onChange={(e) => setDescription(e.target.value)}
              placeholder="Describe your project, its goals, and its intended impact..."
          ...
          >
            {isLoading ? <><LoaderIcon className="h-5 w-5" /> Writing...</> : <><SparklesIcon className="h-5 w-5" /> Write Proposal</>}
          </button>
        </div>
        ...
        <div className="bg-white/70 dark:bg-gray-950/30 backdrop-blur-md border border-gray-200 dark:border-gray-800/50 rounded-2xl p-6 shadow-xl dark:shadow-2xl dark:shadow-black/20 flex flex-col min-h-0">
          <h2 className="text-2xl font-semibold text-gray-900 dark:text-white mb-4">Generated Proposal Outline</h2>
          <div className="w-full flex-grow bg-gray-50 dark:bg-black/20 rounded-xl p-4 overflow-y-auto border border-gray-200 dark:border-gray-800">
            {isLoading && (
              <div className="flex flex-col items-center justify-center h-full gap-4"><LoaderIcon className="h-12 w-12 text-indigo-500 dark:text-indigo-400" /><p className="text-indigo-600 dark:text-indigo-300">Drafting your proposal...</p></div>
            )}
            {error && !isLoading && (
            ...
            {!isLoading && !error && proposal && (
              <div className="prose dark:prose-invert max-w-none whitespace-pre-wrap">{proposal}</div>
            )}
            {!isLoading && !error && !proposal && (
              <div className="flex items-center justify-center h-full text-center text-gray-400 dark:text-gray-600">
                <div><GrantProposalWriterIcon className="h-24 w-24 mx-auto mb-4" /><p>Your grant proposal outline will appear here.</p></div>
              </div>
            )}
          </div>
        </div>
...
export default GrantProposalWriter;
