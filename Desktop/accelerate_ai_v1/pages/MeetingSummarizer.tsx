...
import { BriefcaseIcon, LoaderIcon, AlertTriangleIcon, SparklesIcon } from '../components/icons';
...
const MeetingSummarizer: React.FC = () => {
  ...
  const isSummarizeDisabled = useMemo(() => isLoading || !transcript.trim(), [isLoading, transcript]);

  return (
    <div className="h-full flex flex-col">
      <header className="flex items-center gap-4 mb-4">
        <BriefcaseIcon className="h-10 w-10 text-indigo-500 dark:text-indigo-400" />
        <h1 className="text-4xl font-bold text-gray-900 dark:text-slate-100">Meeting Summarizer</h1>
      </header>
      <p className="text-lg text-gray-600 dark:text-slate-400 mb-6 max-w-4xl">
        Transform long meeting transcripts into concise summaries. Paste your transcript to extract key decisions and action items instantly.
      </p>
      ...
      <main className="flex-grow grid grid-cols-1 lg:grid-cols-2 gap-8 min-h-0">
        <div className="bg-white/70 dark:bg-slate-800/50 backdrop-blur-md border border-gray-200 dark:border-slate-700 rounded-2xl p-6 shadow-xl dark:shadow-2xl dark:shadow-black/20 flex flex-col">
          <h2 className="text-2xl font-semibold text-gray-900 dark:text-slate-100 mb-4">Meeting Transcript</h2>
          <div className="relative w-full flex-grow">
            ...
            <div className="absolute bottom-3 right-3">
                <VoiceInputButton onTranscript={(t) => setTranscript(current => current + ' ' + t)} />
            </div>
          </div>
          <button
        ...
        <div className="bg-white/70 dark:bg-slate-800/50 backdrop-blur-md border border-gray-200 dark:border-slate-700 rounded-2xl p-6 shadow-xl dark:shadow-2xl dark:shadow-black/20 flex flex-col min-h-0">
          <h2 className="text-2xl font-semibold text-gray-900 dark:text-slate-100 mb-4">Summary & Action Items</h2>
          <div className="w-full flex-grow bg-gray-50 dark:bg-slate-900 rounded-xl p-4 overflow-y-auto border border-gray-200 dark:border-slate-700">
            {isLoading && (
              <div className="flex flex-col items-center justify-center h-full gap-4">
                ...
                <p className="text-indigo-600 dark:text-indigo-300">Generating summary...</p>
              </div>
            )}
            ...
            {!isLoading && !error && !summary && (
              <div className="flex items-center justify-center h-full text-center text-gray-500 dark:text-slate-500">
                <div>
                  <BriefcaseIcon className="h-24 w-24 mx-auto mb-4" />
