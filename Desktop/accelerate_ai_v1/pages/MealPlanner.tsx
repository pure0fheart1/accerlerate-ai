...
import { TableCellsIcon, LoaderIcon, AlertTriangleIcon, SparklesIcon } from '../components/icons';
import { planMeal } from '../services/geminiService';
...
const MealPlanner: React.FC = () => {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    setPlan(null);
    try {
      const result = await planMeal(prompt);
      setPlan(result);
    } catch (err) {
  ...
  return (
    <div className="h-full flex flex-col">
      <header className="flex items-center gap-4 mb-4">
        <TableCellsIcon className="h-10 w-10 text-indigo-500 dark:text-indigo-400" />
      ...
      <p className="text-lg text-gray-600 dark:text-gray-400 mb-6 max-w-4xl">
        Generate weekly meal plans based on your dietary needs, preferences, and goals.
      </p>
      ...
      <main className="flex-grow grid grid-cols-1 lg:grid-cols-3 gap-8 min-h-0">
        <div className="lg:col-span-1 bg-white/70 dark:bg-gray-950/30 backdrop-blur-md border border-gray-200 dark:border-gray-800/50 rounded-2xl p-6 shadow-xl dark:shadow-2xl dark:shadow-black/20 flex flex-col gap-6">
          <h2 className="text-2xl font-semibold text-gray-900 dark:text-white">Dietary Preferences</h2>
          <div className="relative flex-grow">
            <label htmlFor="prompt" className="block text-sm font-medium text-gray-700 dark:text-gray-400 mb-2">Describe your diet</label>
            <div className="relative h-full">
              <textarea
                id="prompt"
                value={prompt}
                onChange={(e) => setPrompt(e.target.value)}
                placeholder="e.g., 'A high-protein vegan diet for someone who works out 4 times a week. I don't like mushrooms.'"
        ...
        <div className="lg:col-span-2 bg-white/70 dark:bg-gray-950/30 backdrop-blur-md border border-gray-200 dark:border-gray-800/50 rounded-2xl p-6 shadow-xl dark:shadow-2xl dark:shadow-black/20 flex flex-col min-h-0">
          <h2 className="text-2xl font-semibold text-gray-900 dark:text-white mb-4">Your Meal Plan</h2>
          <div className="w-full flex-grow bg-gray-50 dark:bg-black/20 rounded-xl p-4 overflow-y-auto border border-gray-200 dark:border-gray-800">
            {isLoading && (
              <div className="flex flex-col items-center justify-center h-full gap-4"><LoaderIcon className="h-12 w-12 text-indigo-500 dark:text-indigo-400" /><p className="text-indigo-600 dark:text-indigo-300">Creating your meal plan...</p></div>
            )}
            {error && !isLoading && (
            ...
            {!isLoading && !error && plan && (
              <div className="space-y-4">
                <h3 className="text-2xl font-bold text-indigo-600 dark:text-indigo-400 text-center mb-4">{plan.planTitle}</h3>
                {plan.dailyPlan.map((day, i) => (
                  <div key={i} className="bg-white/50 dark:bg-gray-900/50 p-4 rounded-lg border border-gray-200 dark:border-gray-700">
                    <h4 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">Day {day.day}</h4>
            ...
            {!isLoading && !error && !plan && (
              <div className="flex items-center justify-center h-full text-center text-gray-400 dark:text-gray-600">
                <div><TableCellsIcon className="h-24 w-24 mx-auto mb-4" /><p>Your personalized meal plan will appear here.</p></div>
              </div>
            )}
          </div>
        </div>
