...
import { ElevatorPitchIcon, LoaderIcon, AlertTriangleIcon, SparklesIcon } from '../components/icons';
import { craftElevatorPitch } from '../services/geminiService';
import { VoiceInputButton } from '../components/VoiceInputButton';

const ElevatorPitchCrafter: React.FC = () => {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    setPitch(null);
    try {
      const result = await craftElevatorPitch(idea);
      setPitch(result);
    } catch (err) {
  ...
  return (
    <div className="h-full flex flex-col">
      <header className="flex items-center gap-4 mb-4">
        <ElevatorPitchIcon className="h-10 w-10 text-indigo-500 dark:text-indigo-400" />
        <h1 className="text-4xl font-bold text-gray-900 dark:text-slate-100">Elevator Pitch Crafter</h1>
      </header>
      <p className="text-lg text-gray-600 dark:text-slate-400 mb-6 max-w-4xl">
        Turn your business idea into a concise and compelling 30-second elevator pitch.
      </p>
      ...
      <main className="flex-grow grid grid-cols-1 lg:grid-cols-2 gap-8 min-h-0">
        <div className="bg-white/70 dark:bg-slate-800/50 backdrop-blur-md border border-gray-200 dark:border-slate-700 rounded-2xl p-6 shadow-xl dark:shadow-2xl dark:shadow-black/20 flex flex-col">
          ...
          <div className="relative w-full flex-grow">
            <textarea
              ...
              placeholder="Describe your business, product, or idea in a few sentences..."
            ...
            <div className="absolute bottom-3 right-3">
                <VoiceInputButton onTranscript={(t) => setIdea(current => (current ? current.trim() + ' ' : '') + t)} />
            </div>
          </div>
          <button
          ...
          >
            {isLoading ? <><LoaderIcon className="h-5 w-5" /> Crafting...</> : <><SparklesIcon className="h-5 w-5" /> Craft Pitch</>}
          </button>
        </div>
        ...
        <div className="bg-white/70 dark:bg-slate-800/50 backdrop-blur-md border border-gray-200 dark:border-slate-700 rounded-2xl p-6 shadow-xl dark:shadow-2xl dark:shadow-black/20 flex flex-col min-h-0">
          <h2 className="text-2xl font-semibold text-gray-900 dark:text-slate-100 mb-4">Generated Pitch</h2>
          <div className="w-full flex-grow bg-gray-50 dark:bg-slate-900 rounded-xl p-4 overflow-y-auto border border-gray-200 dark:border-slate-700">
            {isLoading && (
              <div className="flex flex-col items-center justify-center h-full gap-4">
                ...
                <p className="text-indigo-600 dark:text-indigo-300">Crafting your pitch...</p>
              </div>
            )}
            {error && !isLoading && (
            ...
            {!isLoading && !error && pitch && (
              <div className="prose dark:prose-invert max-w-none text-lg leading-relaxed">{pitch}</div>
            )}
            {!isLoading && !error && !pitch && (
              <div className="flex items-center justify-center h-full text-center text-gray-500 dark:text-slate-500">
                <div>
                  <ElevatorPitchIcon className="h-16 w-16 mx-auto mb-4" />
                  <p>Your generated elevator pitch will appear here.</p>
                </div>
              </div>
            )}
          </div>
...
export default ElevatorPitchCrafter;
