...
import { MagicSystemCreatorIcon, LoaderIcon, AlertTriangleIcon, SparklesIcon } from '../components/icons';
import { createMagicSystem } from '../services/geminiService';
import { VoiceInputButton } from '../components/VoiceInputButton';

const MagicSystemCreator: React.FC = () => {
  ...
  const handleGenerate = useCallback(async () => {
    if (!prompt.trim()) {
      setError("Please provide a prompt for your magic system.");
      return;
    }
    setIsLoading(true);
    setError(null);
    setSystem(null);
    try {
      const result = await createMagicSystem(prompt);
      setSystem(result);
    } catch (err) {
  ...
  return (
    <div className="h-full flex flex-col">
      <header className="flex items-center gap-4 mb-4">
        <MagicSystemCreatorIcon className="h-10 w-10 text-indigo-500 dark:text-indigo-400" />
        <h1 className="text-4xl font-bold text-gray-900 dark:text-slate-100">Magic System Creator</h1>
      </header>
      <p className="text-lg text-gray-600 dark:text-slate-400 mb-6 max-w-4xl">
        Brainstorm unique and compelling magic systems for your fantasy world. Describe a core concept, and AI will flesh out the rules, costs, and limitations.
      </p>
      ...
      <main className="flex-grow grid grid-cols-1 lg:grid-cols-2 gap-8 min-h-0">
        <div className="bg-white/70 dark:bg-slate-800/50 backdrop-blur-md border border-gray-200 dark:border-slate-700 rounded-2xl p-6 shadow-xl dark:shadow-2xl dark:shadow-black/20 flex flex-col">
          <h2 className="text-2xl font-semibold text-gray-900 dark:text-slate-100 mb-4">Core Concept</h2>
          <div className="relative w-full flex-grow">
            <textarea
              value={prompt}
              onChange={(e) => setPrompt(e.target.value)}
              placeholder="e.g., 'A magic system based on emotions', 'Magic drawn from constellations', 'Technology that mimics magic'"
              ...
              aria-label="Magic system concept"
            />
          ...
          >
            {isLoading ? <><LoaderIcon className="h-5 w-5" /> Creating...</> : <><SparklesIcon className="h-5 w-5" /> Create System</>}
          </button>
        </div>
        ...
        <div className="bg-white/70 dark:bg-slate-800/50 backdrop-blur-md border border-gray-200 dark:border-slate-700 rounded-2xl p-6 shadow-xl dark:shadow-2xl dark:shadow-black/20 flex flex-col min-h-0">
          <h2 className="text-2xl font-semibold text-gray-900 dark:text-slate-100 mb-4">Generated Magic System</h2>
          <div className="w-full flex-grow bg-gray-50 dark:bg-slate-900 rounded-xl p-4 overflow-y-auto border border-gray-200 dark:border-slate-700">
            {isLoading && (
              <div className="flex flex-col items-center justify-center h-full gap-4">
                ...
                <p className="text-indigo-600 dark:text-indigo-300">Brewing up some magic...</p>
              </div>
            )}
            {error && !isLoading && (
            ...
            {!isLoading && !error && system && (
              <div className="prose dark:prose-invert max-w-none">{system}</div>
            )}
            {!isLoading && !error && !system && (
              <div className="flex items-center justify-center h-full text-center text-gray-500 dark:text-slate-500">
                <div>
                  <MagicSystemCreatorIcon className="h-24 w-24 mx-auto mb-4" />
                  <p>Your unique magic system will appear here.</p>
                </div>
              </div>
            )}
          </div>
...
export default MagicSystemCreator;
