...
import { UnitTestGeneratorIcon, LoaderIcon, AlertTriangleIcon, SparklesIcon } from '../components/icons';
import { generateUnitTests } from '../services/geminiService';
import { VoiceInputButton } from '../components/VoiceInputButton';

const UnitTestGenerator: React.FC = () => {
  ...
  const handleGenerate = useCallback(async () => {
    if (!code.trim()) {
      setError("Please paste a code snippet to generate tests for.");
      return;
    }
    setIsLoading(true);
    setError(null);
  ...
  return (
    <div className="h-full flex flex-col">
      <header className="flex items-center gap-4 mb-4">
        <UnitTestGeneratorIcon className="h-10 w-10 text-indigo-500 dark:text-indigo-400" />
        <h1 className="text-4xl font-bold text-gray-900 dark:text-white">Unit Test Generator</h1>
      </header>
      <p className="text-lg text-gray-600 dark:text-gray-400 mb-6 max-w-4xl">
        Automatically generate unit tests for your code functions to ensure quality and cover edge cases.
      </p>
      ...
      <main className="flex-grow grid grid-cols-1 lg:grid-cols-2 gap-8 min-h-0">
        <div className="bg-white/70 dark:bg-gray-950/30 backdrop-blur-md border border-gray-200 dark:border-gray-800/50 rounded-2xl p-6 shadow-xl dark:shadow-2xl dark:shadow-black/20 flex flex-col">
          <h2 className="text-2xl font-semibold text-gray-900 dark:text-white mb-4">Code to Test</h2>
          ...
          >
            {isLoading ? <><LoaderIcon className="h-5 w-5" /> Generating...</> : <><SparklesIcon className="h-5 w-5" /> Generate Tests</>}
          </button>
        </div>
        ...
        <div className="bg-white/70 dark:bg-gray-950/30 backdrop-blur-md border border-gray-200 dark:border-gray-800/50 rounded-2xl p-6 shadow-xl dark:shadow-2xl dark:shadow-black/20 flex flex-col min-h-0">
          <h2 className="text-2xl font-semibold text-gray-900 dark:text-white mb-4">Generated Unit Tests</h2>
          <div className="w-full flex-grow bg-gray-50 dark:bg-black/20 rounded-xl p-4 overflow-y-auto border border-gray-200 dark:border-gray-800">
            {isLoading && (
              <div className="flex flex-col items-center justify-center h-full gap-4"><LoaderIcon className="h-12 w-12 text-indigo-500 dark:text-indigo-400" /><p className="text-indigo-600 dark:text-indigo-300">Writing unit tests...</p></div>
            )}
            {error && !isLoading && (
            ...
            {!isLoading && !error && tests && (
              <div className="prose dark:prose-invert max-w-none whitespace-pre-wrap">{tests}</div>
            )}
            {!isLoading && !error && !tests && (
              <div className="flex items-center justify-center h-full text-center text-gray-400 dark:text-gray-600">
                <div><UnitTestGeneratorIcon className="h-24 w-24 mx-auto mb-4" /><p>Your generated unit tests will appear here.</p></div>
              </div>
            )}
          </div>
        </div>
