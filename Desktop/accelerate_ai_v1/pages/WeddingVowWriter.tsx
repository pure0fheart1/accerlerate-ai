...
import { WeddingVowWriterIcon, LoaderIcon, AlertTriangleIcon, SparklesIcon } from '../components/icons';
import { writeWeddingVow } from '../services/geminiService';
import { VoiceInputButton } from '../components/VoiceInputButton';

const WeddingVowWriter: React.FC = () => {
  const [prompt, setPrompt] = useState('');
  const [vows, setVows] = useState<string | null>(null);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const handleGenerate = useCallback(async () => {
    if (!prompt.trim()) {
      setError("Please provide some details about your partner and relationship.");
      return;
    }
    setIsLoading(true);
    setError(null);
    setVows(null);
    try {
      const result = await writeWeddingVow(prompt);
      setVows(result);
    } catch (err) {
  ...
  return (
    <div className="h-full flex flex-col">
      <header className="flex items-center gap-4 mb-4">
        <WeddingVowWriterIcon className="h-10 w-10 text-indigo-500 dark:text-indigo-400" />
        <h1 className="text-4xl font-bold text-gray-900 dark:text-slate-100">Wedding Vow Writer</h1>
      </header>
      <p className="text-lg text-gray-600 dark:text-slate-400 mb-6 max-w-4xl">
        Get help writing personal and heartfelt wedding vows. Share some details about your partner and relationship, and let AI help you find the right words.
      </p>
      ...
      <main className="flex-grow grid grid-cols-1 lg:grid-cols-2 gap-8 min-h-0">
        <div className="bg-white/70 dark:bg-slate-800/50 backdrop-blur-md border border-gray-200 dark:border-slate-700 rounded-2xl p-6 shadow-xl dark:shadow-2xl dark:shadow-black/20 flex flex-col">
          <h2 className="text-2xl font-semibold text-gray-900 dark:text-slate-100 mb-4">Your Thoughts & Feelings</h2>
          <div className="relative w-full flex-grow">
            <textarea
              value={prompt}
              onChange={(e) => setPrompt(e.target.value)}
              placeholder="Describe your partner, your relationship, special memories, what you love about them, and your promises for the future..."
          ...
          >
            {isLoading ? <><LoaderIcon className="h-5 w-5" /> Writing...</> : <><SparklesIcon className="h-5 w-5" /> Write Vows</>}
          </button>
        </div>
        ...
        <div className="bg-white/70 dark:bg-slate-800/50 backdrop-blur-md border border-gray-200 dark:border-slate-700 rounded-2xl p-6 shadow-xl dark:shadow-2xl dark:shadow-black/20 flex flex-col min-h-0">
          <h2 className="text-2xl font-semibold text-gray-900 dark:text-slate-100 mb-4">Generated Vows</h2>
          <div className="w-full flex-grow bg-gray-50 dark:bg-slate-900 rounded-xl p-4 overflow-y-auto border border-gray-200 dark:border-slate-700">
            {isLoading && (
              <div className="flex flex-col items-center justify-center h-full gap-4">
                ...
                <p className="text-indigo-600 dark:text-indigo-300">Writing from the heart...</p>
              </div>
            )}
            {error && !isLoading && (
            ...
            {!isLoading && !error && vows && (
              <div className="prose dark:prose-invert max-w-none whitespace-pre-wrap">{vows}</div>
            )}
            {!isLoading && !error && !vows && (
              <div className="flex items-center justify-center h-full text-center text-gray-500 dark:text-slate-500">
                <div>
                  <WeddingVowWriterIcon className="h-24 w-24 mx-auto mb-4" />
                  <p>Your heartfelt wedding vows will appear here.</p>
                </div>
              </div>
            )}
          </div>
...
export default WeddingVowWriter;
