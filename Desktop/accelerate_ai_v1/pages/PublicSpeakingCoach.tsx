...
import { PublicSpeakingCoachIcon, LoaderIcon, AlertTriangleIcon, SparklesIcon } from '../components/icons';
import { coachPublicSpeaking } from '../services/geminiService';
import { VoiceInputButton } from '../components/VoiceInputButton';

const PublicSpeakingCoach: React.FC = () => {
  ...
  const handleGenerate = useCallback(async () => {
    if (!speech.trim()) {
      setError("Please enter a speech to get feedback.");
      return;
    }
    setIsLoading(true);
    setError(null);
    setFeedback(null);
    try {
      const result = await coachPublicSpeaking(speech);
      setFeedback(result);
    } catch (err) {
  ...
  const isGenerateDisabled = useMemo(() => isLoading || !speech.trim(), [isLoading, speech]);

  return (
    <div className="h-full flex flex-col">
      <header className="flex items-center gap-4 mb-4">
        <PublicSpeakingCoachIcon className="h-10 w-10 text-indigo-500 dark:text-indigo-400" />
        <h1 className="text-4xl font-bold text-gray-900 dark:text-slate-100">Public Speaking Coach</h1>
      </header>
      <p className="text-lg text-gray-600 dark:text-slate-400 mb-6 max-w-4xl">
        Get AI-powered feedback on your speeches. Paste your draft to receive analysis on structure, clarity, impact, and suggestions for improvement.
      </p>
      ...
      <main className="flex-grow grid grid-cols-1 lg:grid-cols-2 gap-8 min-h-0">
        <div className="bg-white/70 dark:bg-slate-800/50 backdrop-blur-md border border-gray-200 dark:border-slate-700 rounded-2xl p-6 shadow-xl dark:shadow-2xl dark:shadow-black/20 flex flex-col">
          <h2 className="text-2xl font-semibold text-gray-900 dark:text-slate-100 mb-4">Your Speech Draft</h2>
          <div className="relative w-full flex-grow">
            <textarea
              value={speech}
              onChange={(e) => setSpeech(e.target.value)}
              placeholder="Paste your full speech text here..."
              ...
              aria-label="Speech draft input"
            />
            <div className="absolute bottom-3 right-3">
                <VoiceInputButton onTranscript={(t) => setSpeech(current => (current ? current.trim() + ' ' : '') + t)} />
            </div>
          </div>
          <button
          ...
          <div className="w-full flex-grow bg-gray-50 dark:bg-slate-900 rounded-xl p-4 overflow-y-auto border border-gray-200 dark:border-slate-700">
            {isLoading && (
              <div className="flex flex-col items-center justify-center h-full gap-4">
                ...
                <p className="text-indigo-600 dark:text-indigo-300">Analyzing your speech...</p>
              </div>
            )}
            {error && !isLoading && (
            ...
            {!isLoading && !error && feedback && (
              <div className="prose dark:prose-invert max-w-none">{feedback}</div>
            )}
            {!isLoading && !error && !feedback && (
              <div className="flex items-center justify-center h-full text-center text-gray-500 dark:text-slate-500">
                <div>
                  <PublicSpeakingCoachIcon className="h-24 w-24 mx-auto mb-4" />
                  <p>Feedback on your speech will appear here.</p>
                </div>
              </div>
            )}
          </div>
...
export default PublicSpeakingCoach;
