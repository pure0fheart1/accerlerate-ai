...
import { BrandNameIcon, LoaderIcon, AlertTriangleIcon, SparklesIcon } from '../components/icons';
import { generateBrandNames } from '../services/geminiService';
import { VoiceInputButton } from '../components/VoiceInputButton';

const DESCRIPTION_KEY = 'accelerate-brandname-description';
const OUTPUT_KEY = 'accelerate-brandname-output';

const BrandNameGenerator: React.FC = () => {
  const [description, setDescription] = useState(() => localStorage.getItem(DESCRIPTION_KEY) || '');
  const [names, setNames] = useState<string | null>(() => localStorage.getItem(OUTPUT_KEY) || null);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => { localStorage.setItem(DESCRIPTION_KEY, description); }, [description]);
  useEffect(() => {
    if (names) localStorage.setItem(OUTPUT_KEY, names);
    else localStorage.removeItem(OUTPUT_KEY);
  }, [names]);

  const handleGenerate = useCallback(async () => {
    if (!description.trim()) {
      setError("Please describe your company or product.");
      return;
    }
    setIsLoading(true);
    setError(null);
    setNames(null);
    try {
      const result = await generateBrandNames(description);
      setNames(result);
    } catch (err) {
  ...
  return (
    <div className="h-full flex flex-col">
      <header className="flex items-center gap-4 mb-4">
        <BrandNameIcon className="h-10 w-10 text-indigo-500 dark:text-indigo-400" />
        <h1 className="text-4xl font-bold text-gray-900 dark:text-white">Brand Name Generator</h1>
      </header>
      <p className="text-lg text-gray-600 dark:text-gray-400 mb-6 max-w-4xl">
        Brainstorm unique and catchy names for your brand or product.
      </p>
      ...
      <main className="flex-grow grid grid-cols-1 lg:grid-cols-3 gap-8 min-h-0">
        <div className="lg:col-span-1 bg-white/70 dark:bg-gray-950/30 backdrop-blur-md border border-gray-200 dark:border-gray-800/50 rounded-2xl p-6 shadow-xl dark:shadow-2xl dark:shadow-black/20 flex flex-col gap-6">
          <h2 className="text-2xl font-semibold text-gray-900 dark:text-white">Company Description</h2>
          <div className="relative flex-grow">
            <label htmlFor="description" className="block text-sm font-medium text-gray-700 dark:text-gray-400 mb-2">Describe what your company does</label>
             <div className="relative h-full">
                <textarea
                  id="description"
                  value={description}
                  onChange={(e) => setDescription(e.target.value)}
                  placeholder="e.g., 'A subscription box for eco-friendly dog toys', 'A mobile app that connects local musicians'"
          ...
          >
            {isLoading ? <><LoaderIcon className="h-5 w-5" /> Generating...</> : <><SparklesIcon className="h-5 w-5" /> Generate Names</>}
          </button>
        </div>
        ...
        <div className="lg:col-span-2 bg-white/70 dark:bg-gray-950/30 backdrop-blur-md border border-gray-200 dark:border-gray-800/50 rounded-2xl p-6 shadow-xl dark:shadow-2xl dark:shadow-black/20 flex flex-col min-h-0">
          <h2 className="text-2xl font-semibold text-gray-900 dark:text-white mb-4">Generated Names</h2>
          <div className="w-full flex-grow bg-gray-50 dark:bg-black/20 rounded-xl p-4 overflow-y-auto border border-gray-200 dark:border-gray-800">
            {isLoading && (
              <div className="flex flex-col items-center justify-center h-full gap-4"><LoaderIcon className="h-12 w-12 text-indigo-500 dark:text-indigo-400" /><p className="text-indigo-600 dark:text-indigo-300">Brainstorming names...</p></div>
            )}
            {error && !isLoading && (
            ...
            {!isLoading && !error && names && (
              <div className="prose dark:prose-invert max-w-none whitespace-pre-wrap">{names}</div>
            )}
            {!isLoading && !error && !names && (
              <div className="flex items-center justify-center h-full text-center text-gray-400 dark:text-gray-600">
                <div><BrandNameIcon className="h-24 w-24 mx-auto mb-4" /><p>Your generated brand names will appear here.</p></div>
              </div>
            )}
          </div>
        </div>
