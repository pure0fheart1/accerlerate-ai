...
import { LightBulbIcon, LoaderIcon, AlertTriangleIcon, SparklesIcon } from '../components/icons';
  ...
  return (
    <div className="h-full flex flex-col">
      <header className="flex items-center gap-4 mb-4">
        <LightBulbIcon className="h-10 w-10 text-indigo-500 dark:text-indigo-400" />
        <h1 className="text-4xl font-bold text-gray-900 dark:text-slate-100">Idea Generator</h1>
      </header>
      <p className="text-lg text-gray-600 dark:text-slate-400 mb-6 max-w-4xl">
        Overcome creative blocks by generating unique ideas for any topic or problem.
      </p>
      ...
      <div className="bg-white/70 dark:bg-slate-800/50 backdrop-blur-md border border-gray-200 dark:border-slate-700 rounded-2xl p-6 shadow-xl dark:shadow-2xl dark:shadow-black/20 mb-8 flex flex-col sm:flex-row items-center gap-4">
        <div className="relative w-full flex-grow">
          <input
            ...
            placeholder="e.g., 'Creative marketing ideas for a coffee shop', 'App ideas for sustainable living'"
            ...
            aria-label="Topic for idea generation"
          />
        ...
        >
          {isLoading ? <><LoaderIcon className="h-5 w-5" /> Brainstorming...</> : <><SparklesIcon className="h-5 w-5" /> Generate Ideas</>}
        </button>
      </div>

      <main className="flex-grow flex flex-col min-h-0">
        <h2 className="text-3xl font-bold text-gray-900 dark:text-slate-100 mb-4">Generated Ideas</h2>
        <div className="w-full flex-grow bg-gray-50 dark:bg-slate-900 rounded-xl p-4 overflow-y-auto border border-gray-200 dark:border-slate-700">
            {isLoading && (
              <div className="flex flex-col items-center justify-center h-full gap-4">
                ...
                <p className="text-indigo-600 dark:text-indigo-300">Generating creative ideas...</p>
              </div>
            )}
            {error && !isLoading && (
            ...
            {!isLoading && !error && !ideas && (
              <div className="flex items-center justify-center h-full text-center text-gray-500 dark:text-slate-500">
                <div>
                  <LightBulbIcon className="h-24 w-24 mx-auto mb-4" />
