...
import { RecipeNutritionAnalyzerIcon, LoaderIcon, AlertTriangleIcon, SparklesIcon } from '../components/icons';
import { analyzeRecipeNutrition } from '../services/geminiService';
import { VoiceInputButton } from '../components/VoiceInputButton';

const RecipeNutritionAnalyzer: React.FC = () => {
  ...
  const handleGenerate = useCallback(async () => {
    if (!recipe.trim()) {
      setError("Please paste a recipe to analyze.");
      return;
    }
    setIsLoading(true);
    setError(null);
  ...
  const isGenerateDisabled = useMemo(() => isLoading || !recipe.trim(), [isLoading, recipe]);

  return (
    <div className="h-full flex flex-col">
      <header className="flex items-center gap-4 mb-4">
        <RecipeNutritionAnalyzerIcon className="h-10 w-10 text-indigo-500 dark:text-indigo-400" />
        <h1 className="text-4xl font-bold text-gray-900 dark:text-white">Recipe Nutrition Analyzer</h1>
      </header>
      <p className="text-lg text-gray-600 dark:text-gray-400 mb-6 max-w-4xl">
        Get an estimated nutritional breakdown for any recipe. Paste the ingredients and instructions to get started. <span className="font-semibold text-yellow-500 dark:text-yellow-400">Note: This is an estimate, not a substitute for professional advice.</span>
      </p>
      ...
      <main className="flex-grow grid grid-cols-1 lg:grid-cols-2 gap-8 min-h-0">
        <div className="bg-white/70 dark:bg-gray-950/30 backdrop-blur-md border border-gray-200 dark:border-gray-800/50 rounded-2xl p-6 shadow-xl dark:shadow-2xl dark:shadow-black/20 flex flex-col">
          <h2 className="text-2xl font-semibold text-gray-900 dark:text-white mb-4">Recipe to Analyze</h2>
          <div className="relative w-full flex-grow">
            ...
            <div className="absolute bottom-3 right-3">
                <VoiceInputButton onTranscript={(t) => setRecipe(current => current + '\n' + t)} />
            </div>
          </div>
          <button
          ...
          >
            {isLoading ? <><LoaderIcon className="h-5 w-5" /> Analyzing...</> : <><SparklesIcon className="h-5 w-5" /> Analyze Recipe</>}
          </button>
        </div>
        ...
        <div className="bg-white/70 dark:bg-gray-950/30 backdrop-blur-md border border-gray-200 dark:border-gray-800/50 rounded-2xl p-6 shadow-xl dark:shadow-2xl dark:shadow-black/20 flex flex-col min-h-0">
          ...
          <div className="w-full flex-grow bg-gray-50 dark:bg-black/20 rounded-xl p-4 overflow-y-auto border border-gray-200 dark:border-gray-800">
            {isLoading && (
              <div className="flex flex-col items-center justify-center h-full gap-4"><LoaderIcon className="h-12 w-12 text-indigo-500 dark:text-indigo-400" /><p className="text-indigo-600 dark:text-indigo-300">Calculating nutrition...</p></div>
            )}
            {error && !isLoading && (
            ...
            {!isLoading && !error && !analysis && (
              <div className="flex items-center justify-center h-full text-center text-gray-400 dark:text-gray-600">
                <div><RecipeNutritionAnalyzerIcon className="h-24 w-24 mx-auto mb-4" /><p>Your recipe's nutritional analysis will appear here.</p></div>
              </div>
            )}
          </div>
        </div>
...
export default RecipeNutritionAnalyzer;
