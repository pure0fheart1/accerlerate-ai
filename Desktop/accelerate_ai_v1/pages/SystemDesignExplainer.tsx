...
import { SystemDesignExplainerIcon, LoaderIcon, AlertTriangleIcon, SparklesIcon } from '../components/icons';
import { explainSystemDesign } from '../services/geminiService';
import { VoiceInputButton } from '../components/VoiceInputButton';

const SystemDesignExplainer: React.FC = () => {
  ...
  const handleGenerate = useCallback(async () => {
    if (!concept.trim()) {
      setError("Please enter a system design concept to explain.");
      return;
    }
    setIsLoading(true);
    setError(null);
    setExplanation(null);
    try {
      const result = await explainSystemDesign(concept);
  ...
  const isGenerateDisabled = useMemo(() => isLoading || !concept.trim(), [isLoading, concept]);

  return (
    <div className="h-full flex flex-col">
      <header className="flex items-center gap-4 mb-4">
        <SystemDesignExplainerIcon className="h-10 w-10 text-indigo-500 dark:text-indigo-400" />
        <h1 className="text-4xl font-bold text-gray-900 dark:text-slate-100">System Design Explainer</h1>
      </header>
      <p className="text-lg text-gray-600 dark:text-slate-400 mb-6 max-w-4xl">
        Get simple, high-level explanations of complex system design concepts and architectures.
      </p>
      ...
      <div className="bg-white/70 dark:bg-slate-800/50 backdrop-blur-md border border-gray-200 dark:border-slate-700 rounded-2xl p-6 shadow-xl dark:shadow-2xl dark:shadow-black/20 mb-8 flex flex-col sm:flex-row items-center gap-4">
        <div className="relative w-full flex-grow">
          <input
            ...
            placeholder="e.g., 'Load Balancer', 'Database Sharding', 'Event-driven architecture'"
            ...
            aria-label="System design concept"
          />
           <div className="absolute inset-y-0 right-0 flex items-center pr-3">
            <VoiceInputButton onTranscript={setConcept} />
          </div>
        </div>
        <button
      ...
      <main className="flex-grow flex flex-col min-h-0">
        <h2 className="text-3xl font-bold text-gray-900 dark:text-slate-100 mb-4">Explanation</h2>
        <div className="w-full flex-grow bg-white/70 dark:bg-slate-800/50 backdrop-blur-md border border-gray-200 dark:border-slate-700 rounded-xl p-4 overflow-y-auto shadow-inner">
            {isLoading && (
              <div className="flex flex-col items-center justify-center h-full gap-4">
                ...
                <p className="text-indigo-600 dark:text-indigo-300">Explaining the concept...</p>
              </div>
            )}
            {error && !isLoading && (
            ...
            {!isLoading && !error && explanation && (
              <div className="prose dark:prose-invert max-w-none">{explanation}</div>
            )}
            {!isLoading && !error && !explanation && (
              <div className="flex items-center justify-center h-full text-center text-gray-500 dark:text-slate-500">
                <div>
                  <SystemDesignExplainerIcon className="h-24 w-24 mx-auto mb-4" />
                  <p>Your system design explanation will appear here.</p>
                </div>
              </div>
            )}
          </div>
...
export default SystemDesignExplainer;
