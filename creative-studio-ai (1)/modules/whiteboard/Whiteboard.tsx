...
const Whiteboard: React.FC = () => {
  ...
  // State
  const defaultLayer: Layer = { id: 'layer-0', name: 'Layer 1', visible: true, locked: false, opacity: 1 };
  const [history, setHistory] = useState<HistoryState[]>([{ paths: [], images: [], textObjects: [], layers: [defaultLayer] }]);
  const [historyIndex, setHistoryIndex] = useState(0);
  const currentState = history[historyIndex];
  const [currentLayerId, setCurrentLayerId] = useState(defaultLayer.id);
  ...
  const [eraserPaths, setEraserPaths] = useState<Point[][]>([]);
  ...
  useEffect(() => {
    const autoSave = setInterval(() => {
      ...
      localStorage.setItem('whiteboardState', JSON.stringify({ history: serializableHistory, historyIndex, viewState, currentLayerId }));
    }, 30000);
    return () => clearInterval(autoSave);
  }, [history, historyIndex, viewState, isLoaded, currentState, currentLayerId]);
        ...
        try {
          ...
          const newHistory: HistoryState[] = await Promise.all(
            savedState.history.map(async (state: SerializableHistoryState) => {
              ...
              return { paths: state.paths, images: loadedImages, textObjects: state.textObjects || [], layers: state.layers || [defaultLayer] };
            })
          );
          setHistory(newHistory);
          setHistoryIndex(savedState.historyIndex);
          setViewState(savedState.viewState);
          setCurrentLayerId(savedState.currentLayerId || defaultLayer.id);
        } catch (error) {
  ...
  const redrawCanvas = useCallback(() => {
    ...
    const { paths, images, textObjects, layers } = currentState;
    ...
    // Sort layers by their order (first in array = bottom layer)
    ...
    sortedLayers.forEach(layer => {
      ...
      // Draw text objects on this layer
      textObjects
        .filter(txt => txt.layerId === layer.id && isEditingText?.id !== txt.id)
        .forEach(txt => {
          ctx.save();
      ...
      const allPaths = isDrawingRef.current && currentPathRef.current && currentPathRef.current.layerId === layer.id
        ? [...layerPaths, currentPathRef.current]
        : layerPaths;
  ...
  const handleZoomIn = useCallback(() => {
    setViewState(v => ({ ...v, scale: Math.min(v.scale * 1.2, 5) }));
  }, []);

  const handleZoomOut = useCallback(() => {
    setViewState(v => ({ ...v, scale: Math.max(v.scale / 1.2, 0.1) }));
  }, []);
  ...
  }, [currentState, recordNewState]);

  const deleteLayer = useCallback((layerId: string) => {
    if (currentState.layers.length <= 1) return; // Don't delete the last layer

    const newLayers = currentState.layers.filter(l => l.id !== layerId);
    const newPaths = currentState.paths.filter(p => p.layerId !== layerId);
    const newImages = currentState.images.filter(i => i.layerId !== layerId);
    const newTextObjects = currentState.textObjects.filter(t => t.layerId !== layerId);
  ...
  }, [currentState, recordNewState, currentLayerId]);

  const toggleLayerVisibility = useCallback((layerId: string) => {
    const newLayers = currentState.layers.map(layer =>
      layer.id === layerId ? { ...layer, visible: !layer.visible } : layer
    );
    recordNewState({ ...currentState, layers: newLayers });
  }, [currentState, recordNewState]);
  ...
  // ====================================================================================
  const erasePathsAt = useCallback((point: Point, radius: number) => {
    const remainingPaths: Path[] = [];

    for (const path of currentState.paths) {
      if (path.layerId !== currentLayerId) {
    ...
    if (remainingPaths.length !== currentState.paths.length) {
      updateCurrentState(prev => ({ ...prev, paths: remainingPaths }));
    }
  }, [currentState.paths, currentLayerId, updateCurrentState]);
  ...
  const handleWheelZoom = useCallback((e: React.WheelEvent<HTMLCanvasElement>) => {
    ...
    setViewState(v => {
      const newScale = Math.max(0.1, Math.min(5, v.scale * zoomFactor));
  ...
  const handlePointerDown = (e: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    switch (tool) {
      ...
      case 'eraser':
        isDrawingRef.current = true;
        erasePathsAt(point, strokeWidth);
        currentPathRef.current = null; // No path for eraser
        break;
    }
  };
        ...
        if (selectedObject.type === 'image') {
            ...
            switch (type) {
              ...
              case 'rotating':
                ...
                break;
              case 'resizing':
                const handle = (interactionRef.current as { type: 'resizing'; handle: InteractionHandle }).handle;
                const aspectRatio = obj.width / obj.height;

                switch (handle) {
                  case 'br': // Bottom-right
                    obj.width = Math.max(20, obj.width + dx);
                    obj.height = Math.max(20, obj.width / aspectRatio);
                    break;
                  case 'bl': // Bottom-left
                    const newWidth = Math.max(20, obj.width - dx);
                    const newHeight = Math.max(20, newWidth / aspectRatio);
                    obj.x += obj.width - newWidth;
                    obj.width = newWidth;
                    obj.height = newHeight;
                    break;
                  case 'tr': // Top-right
                    const trWidth = Math.max(20, obj.width + dx);
                    const trHeight = Math.max(20, trWidth / aspectRatio);
                    obj.y += obj.height - trHeight;
                    obj.width = trWidth;
                    obj.height = trHeight;
                    break;
                  case 'tl': // Top-left
                    const tlWidth = Math.max(20, obj.width - dx);
                    const tlHeight = Math.max(20, tlWidth / aspectRatio);
                    obj.x += obj.width - tlWidth;
                    obj.y += obj.height - tlHeight;
                    obj.width = tlWidth;
                    obj.height = tlHeight;
                    break;
                }
            ...
            switch (type) {
              ...
              case 'resizing':
                ...
                switch (handle) {
                  case 'br': // Bottom-right - increase font size
                    obj.fontSize = Math.max(8, obj.fontSize + dx * 0.5);
                    obj.width = Math.max(50, obj.width + dx);
                    break;
                  case 'bl': // Bottom-left - increase font size
                    obj.fontSize = Math.max(8, obj.fontSize - dx * 0.5);
                    const newWidth = Math.max(50, obj.width - dx);
                    obj.x += obj.width - newWidth;
                    obj.width = newWidth;
                    break;
                  case 'tr': // Top-right - increase font size
                    obj.fontSize = Math.max(8, obj.fontSize + dx * 0.5);
                    obj.width = Math.max(50, obj.width + dx);
                    break;
                  case 'tl': // Top-left - increase font size
                    obj.fontSize = Math.max(8, obj.fontSize - dx * 0.5);
                    const tlWidth = Math.max(50, obj.width - dx);
                    obj.x += obj.width - tlWidth;
                    obj.width = tlWidth;
                    break;
                }
  ...
  const handlePointerUp = (e: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    if (isDrawingRef.current) {
        ...
        if (tool === 'eraser') {
            // For eraser, the state was already updated during movement, just commit to history
            recordNewState(history[historyIndex]);
  ...
  const handleClear = () => {
    recordNewState({ paths: [], images: [], textObjects: [], layers: [defaultLayer] });
    setViewState({ scale: 1, offsetX: 0, offsetY: 0 });
    ...
    <div className="flex flex-row h-full w-full gap-4">
      <div ref={containerRef} className="flex-1 h-full bg-slate-900 rounded-lg overflow-hidden relative">
       ...
       <div className="w-56 bg-slate-800 p-4 rounded-lg shadow-lg flex flex-col gap-y-6 overflow-y-auto">
          ...
          <div>
              <h4 className="font-semibold mb-3 text-slate-300 border-b border-slate-700 pb-2">Zoom</h4>
              <div className="flex gap-2 mt-2">
                  <button onClick={handleZoomOut} className="flex-1 px-3 py-2 rounded-md transition-colors bg-slate-700 hover:bg-slate-600">-</button>
                  <button onClick={handleResetZoom} className="flex-1 px-3 py-2 rounded-md transition-colors bg-slate-700 hover:bg-slate-600 text-xs">
                      {Math.round(viewState.scale * 100)}%
                  </button>
                  <button onClick={handleZoomIn} className="flex-1 px-3 py-2 rounded-md transition-colors bg-slate-700 hover:bg-slate-600">+</button>
              </div>
          </div>
          <div>
              <h4 className="font-semibold mb-3 text-slate-300 border-b border-slate-700 pb-2">Layers</h4>
              <div className="flex flex-col gap-2 mt-2">
                  <button onClick={addLayer} className="w-full px-3 py-2 rounded-md transition-colors bg-blue-600 hover:bg-blue-700 text-white">
                      + Add Layer
                  </button>
                  <div className="max-h-32 overflow-y-auto">
                      {currentState.layers.slice().reverse().map((layer, index) => (
                          <div key={layer.id} className={`flex items-center gap-2 p-2 rounded-md ${currentLayerId === layer.id ? 'bg-blue-600' : 'bg-slate-700'}`}>
                              <button
