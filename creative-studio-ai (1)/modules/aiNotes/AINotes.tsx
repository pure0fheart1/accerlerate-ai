...
const AINotes: React.FC = () => {
  ...
  const aiNotes = useMemo(() =>
    savedNotes
      .filter(note => note.destination === 'aiNotes')
      .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()),
    [savedNotes]
  );
  
  const selectedNote = useMemo(() =>
    aiNotes.find(note => note.id === selectedNoteId),
    [aiNotes, selectedNoteId]
  );

  // Automatically select the first note if none is selected
  useState(() => {
    if (!selectedNoteId && aiNotes.length > 0) {
      setSelectedNoteId(aiNotes[0].id);
    }
  });
    ...
    return (
      <div className="flex items-center justify-center h-full">
        <div className="text-center text-slate-500">
          <h2 className="text-2xl font-semibold">No AI Notes Yet</h2>
          <p>Go to Handwritten Notes to save your first AI note.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="flex h-full -m-6">
      ...
      <div className="w-1/3 border-r border-slate-700 bg-slate-800/50 overflow-y-auto">
        <div className="p-4 border-b border-slate-700">
          <h3 className="text-xl font-bold">AI Note Entries</h3>
        </div>
        <ul>
          {aiNotes.map(note => (
            <li key={note.id}>
              <button
                ...
                className={`w-full text-left p-4 border-b border-slate-700 hover:bg-slate-700 transition-colors ${selectedNoteId === note.id ? 'bg-purple-600/30' : ''}`}
              >
